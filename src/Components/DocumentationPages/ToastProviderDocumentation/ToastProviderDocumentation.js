  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Method                                    | Returns    | Description                              |","|-------------------------------------------|------------|------------------------------------------|","| `show(message, config?)`                  | `string`   | Shows a toast, returns its unique id     |","| `dismiss(id)`                             | `this`     | Dismisses the toast with the given id    |","| `clear()`                                 | `this`     | Dismisses all visible toasts             |","| `setPosition(position)`                   | `this`     | Changes the position                     |","| `destroy()`                               | `void`     | Clears all toasts and removes the container |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
