  ...
  async init() {
    ...
    if (true) {
      ...
      }
      {
         ...
         if (container) {
            const lines = ["| Scenario | Use |","|---|---|","| 1–3 tooltips, declarative markup | `<slice-tooltip>` component |","| 10+ tooltips, dynamic content | `ToolTipProvider` |","| Toolbar with icon buttons | `ToolTipProvider.scope()` + data attributes |","| App-wide shared singleton | `ToolTipProvider.getInstance()` |"];
            const clean = (line) => {
               let value = line.trim();
               if (value.startsWith('|')) {
                  value = value.slice(1);
