...
const generateDocumentationRoutesFile = (entries) => {
  ...
  const libraryHomeEntry = entries.find((entry) => entry.route === '/docs');
  const defaultComponent = libraryHomeEntry?.component || entries[0]?.component || 'ButtonDocumentation';
...
export { validateFrontMatter, mergeComponentsRegistry, lintMarkdownSourceForManualPropsTable };
